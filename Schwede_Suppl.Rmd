...
if (length(ReqPkg[!ReqPkg%in%pkg])>0){
...
sapply(geneSig[1:2], dplyr::n_distinct, na.rm=TRUE)
sapply(geneSig[3:4], dplyr::n_distinct, na.rm=TRUE)
...
library(limma)
 ST<-birrer_eSet[,!birrer_eSet$sample_type =="Normal"]  # Subset to just Tumor, Stroma
 Pair<-factor(as.character(ST$alt_sample_name))
 Cell<-factor(as.character(ST$sample_type))
 design<-model.matrix(~Pair+Cell)
 fit_pair<-lmFit(ST,design)
 fit_pair <- eBayes(fit_pair)
res<- topTable(fit_pair, coef="CellTumor", p.value=0.0001, sort.by="p",adjust.method="bonferroni", number=nrow(ST),genelist=featureNames(ST))
summary(decideTests(fit_pair, adjust.method = "bonferroni", p.value = 0.0001))[,"CellTumor"]

BirrerStromaSig<-res[res$adj.P.Val<0.0000000001&res$logFC<0,1]
BirrerTumorSig<-res[res$adj.P.Val<0.0000000001&res$logFC>0,1]
length(BirrerStromaSig)
length(BirrerTumorSig)
...
library(annotate)
library("hgu133plus2.db")
BirrerSig<-list(BirrerStromaSig,BirrerTumorSig)
BirrerSig<-c(BirrerSig,lapply(BirrerSig, function(sig){ symsig<-unique(as.character(getSYMBOL(sig,"hgu133plus2.db")))
   symsig<-symsig[!is.na(symsig)]}))

names(BirrerSig) =names(geneSig)
sapply(BirrerSig,dplyr::n_distinct, na.rm=TRUE)

```

# Adjusting for batch effects. Identify Genes that are associated with Batch
...
ST$batch_yrsMonth= substr(ST$batch,1,7)
ST$batch_yrs= substr(ST$batch,1,4)
design<-model.matrix(~vital_status+batch_yrsMonth, data=ST)
fit<-lmFit(ST,design)
fit <- eBayes(fit)
#summary(decideTests(fit, adjust.method = "BH", p.value = 0.0001))
b<-grep("batch",colnames(coef(fit)))  # Batch Coefficients
...
# Select Stroma Samples
ST<-birrer_eSet[,birrer_eSet$sample_type =="Stroma"]
...
venn(list(Tumor=BirrerSig$Tumor_AffyIDs, Stroma=BirrerSig$Stromal_AffyIDs, batch=c(batchS, batchT)))
```

# Actually Suppl Table
Reading Birrer Sig from Supplemementary Table  
```{r}
BirrerSig.new=BirrerSig
#BirrerSig is computed 

BirrerSig<-supplTabs[[3]]
BirrerSig<-as.list(BirrerSig)
BirrerSig<-lapply(BirrerSig, function(x) x[!is.na(x)])

# Check if any change with annotation update,
# Affy IDs are equal
sapply(BirrerSig[1:2], dplyr::n_distinct, na.rm=TRUE)
all(unlist(BirrerSig$Stromal_AffyIDs) == BirrerSig.new$Stromal_AffyIDs)
all(unlist(BirrerSig$Tumor_AffyIDs) == BirrerSig.new$Tumor_AffyIDs)

sapply(BirrerSig[1:4], dplyr::n_distinct, na.rm=TRUE)
...
MSgenes<-read.csv(file.path(inputdir,"AOCS subtype specific genes.csv"), as.is=TRUE, row.names=1, na.strings = "AAAAAAAAAA")
...
```{r}
# read MGH, AOCS 0.05, 0.001 genelists suppl table
# Table 2
# MSgenes is the molecular subtype genes from the AOCS Tothill suppl 
gg<-readxl::read_xlsx(file.path(outdir,"CCR_SupplementaryTables 2.xlsx"), sheet = 2, skip=4)


ll <-list(AOCS_Stroma=gg$`Stromal gene symbols...3`,
          AOCS_Tumor=gg$`Tumor gene symbols...4`,
          AOCS_Stroma001=gg$`Stromal gene symbols...7`,
         AOCS_Tumor001=  gg$`Tumor gene symbols...8`,
          MGH_Stroma=gg$`Stroma Gene Symbols`, 
         MGH_Tumor= gg$`Tumor Gene Symbols`,
         AOCS_C1=AOCS_sig[AOCS_sig$Subtype=="C1","SYMBOL"],
         C1vC2=MSgenes[,2], C1vC4=MSgenes[,4], C1vC5=MSgenes[,6], C2vC4=MSgenes[,8],C2vC5=MSgenes[,10],C4vC5=MSgenes[,12])

ll<-lapply(ll, function(x) unique(x[!is.na(x)]))
sapply(ll, dplyr::n_distinct)
#AOCS molecular subtypes comparisons
# Table 3
gg2<-readxl::read_xlsx(file.path(outdir,"CCR_SupplementaryTables 2.xlsx"), sheet = 3, skip=4)
...
write.csv(res, file=file.path(outdir, "./results_fisher_overlapGenelists.csv"))
```
